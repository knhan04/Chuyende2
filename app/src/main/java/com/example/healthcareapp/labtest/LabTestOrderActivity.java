    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 2. Gửi lên Server
                RemoteDataBase remoteDb = new RemoteDataBase(LabTestOrderActivity.this);
                remoteDb.addOrder(username, edname.getText().toString(), edAddress.getText().toString(), edContact.getText().toString(), Integer.parseInt(edPin.getText().toString()), date, time, Float.parseFloat(price[1]), "lab test", new RemoteDataBase.DatabaseCallback<Void>() {
                    @Override
                    public void onSuccess(Void result) {
                        Toast.makeText(getApplicationContext(), "Đặt lịch xét nghiệm thành công!", Toast.LENGTH_SHORT).show();
